...
maternal <- read.table(paste(dir,"/data/expression/Maternal_gene_normalized_v19_6.25.17.txt", sep=""), check.names = F)
paternal <- read.table(paste(dir,"/data/expression/Paternal_gene_normalized_v19_6.25.17.txt", sep=""),  check.names = F)
...
write.table(table, "Asymmetry_10000_06.25.17m_all.txt", quote = F, row.names = T, col.names = F)
...
while (length(asym2$pvals) > 10) {
  ...
  write.table(table, paste("Asymmetry_10000_06.25.17m_",t,".txt",sep="") , quote = F, row.names = T, col.names = F)
}
