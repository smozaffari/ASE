...
maternal <- read.table(paste(dir,"/results/tests_asym/Maternal_0.txt", sep=""), check.names = F)
paternal <- read.table(paste(dir,"/results/tests_asym/Paternal_0.txt", sep=""),  check.names = F)
...
asym <- permute2(maternal, paternal, 100000)
table <- cbind(asym$pvals, asym$T, asym$dir)
rownames(table) <- names(asym$pvals)

write.table(table, "Asymmetryround2_100000_07.30.txt", quote = F, row.names = T, col.names = F)
