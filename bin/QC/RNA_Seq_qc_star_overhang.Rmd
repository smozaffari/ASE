...
count <- read.table("~/star_overhang_genecount_withsex", check.names = F)
count[1:5,1:3]
```
...
fcmap <- c()
for (i in 1:11) {
  fcreads <- read.table(paste("~/new_FC_",i,sep=""))
...
counts1 = res2+1
CPM = apply(counts1, 2, function(x) (x*1000000)/sum(x))
hist(log2(CPM), main = "CPM")
...
newres <- res2
sexRatio = log2(newres["XIST",]+1) - log2(newres["EIF1AY",]+1)
...
findivs <- sapply(strsplit(colnames(newres), "_"), "[", 2)
...
beforesumlane <- newres
res3 <- as.matrix(newres)
var <- res3
findivs <- sapply(strsplit(colnames(var), "_"), "[", 2)
colnames(var) <- findivs
nms <- colnames(var)
aftersumlane2 <- var %*% sapply(unique(nms), "==", nms)
aftersumlane<- aftersumlane2[,order(colnames(aftersumlane2))]
...
aftersumlanecov2 <- as.data.frame(t(combinecov %*% sapply(unique(nms), "==", nms)))
#dim(aftersumlanecov)
aftersumlanecov<- aftersumlanecov2[order(rownames(aftersumlanecov2)),]
...
counts1 = after2+1
CPM2 = apply(counts1, 2, function(x) (x*1000000)/sum(x))
hist(log2(CPM2), main = "CPM")
```

##Checking sex after combining replicates

```{r genderaftercombine, echo=FALSE}
head(after2["XIST",1:5])
sexRatio = log2(after2["XIST",]+1) - log2(after2["EIF1AY",]+1)
# if this is > 0, Female, < 0, male based on expression
callSex= ifelse(sexRatio >0, "F", "M")
table(callSex)

findivs <- colnames(after2)
names(sexRatio) <- findivs
gender <- sapply(findivs, function(x) substr(x, nchar(x), nchar(x)))
table(gender)
...
CPMB2 <- CPM[,-c(grep(paste(a, collapse="|"), colnames(CPM)))]
...
res.gx <- aftersumlane
x.overlap<-intersect(rownames(res.gx), chrX.genes)
y.overlap<-intersect(rownames(res.gx), chrY.genes)
m.overlap<-intersect(rownames(res.gx), chrM.genes)

inboth.xchr.a <-match(x.overlap, rownames(after3))
inboth.ychr.a <- match(y.overlap, rownames(after3))

inboth.xchr.b <-match(x.overlap, rownames(before3))
inboth.ychr.b <- match(y.overlap, rownames(before3))

res.gx.aa<-after3[-c(inboth.xchr.a, inboth.ychr.a),]
res.gx.a <- res.gx.aa[rowSums(res.gx.aa)>10,]

CPMAxy<-CPM3[-c(inboth.xchr.a, inboth.ychr.a),]
CPMA<- CPMAxy[rowSums(res.gx.aa)>10,]

res.gx.bb<-before3[-c(inboth.xchr.b, inboth.ychr.b),]
res.gx.b <- res.gx.bb[rowSums(res.gx.bb)>1,]

CPMBxy<-CPMB1[-c(inboth.xchr.b, inboth.ychr.b),]
CPMB<- CPMBxy[rowSums(res.gx.bb)>1,]
```
...
less.one<-NULL
summary.low.count<-matrix(nrow=nrow(CPMA), ncol=1)
for(i in 1:nrow(CPMA)){
  less.one<-NULL
  less.one<-which(CPMA[i,]<1)
  summary.low.count[i,]<-length(less.one)
  rm(less.one)
}
# Determine which samples have count <1 in at least 10 samples: 
keep.genes<-summary.low.count[,1]<10
#table(keep.genes)
#keep.genes
#FALSE  TRUE 
#12045 10293 

# Try with less strict: 20 samples
keep.genes<-summary.low.count[,1]<100
#table(keep.genes)
#100:FALSE  TRUE 
#8585 11334 
...
#CPMA<-CPMA[keep.genes,]
#res.gx.a<-res.gx.a[keep.genes,]


less.one<-NULL
summary.low.count<-matrix(nrow=nrow(CPMB), ncol=1)
for(i in 1:nrow(CPMB)){
  less.one<-NULL
  less.one<-which(CPMB[i,]<1)
  summary.low.count[i,]<-length(less.one)
  rm(less.one)
}
# Determine which samples have count <1 in at least 10 samples: 
...
CPMB<-CPMB[keep.genes,]
#res.gx.b<-res.gx.b[keep.genes,]
...
```{r qc, echo=FALSE, fig.height=5, fig.width=10, eval=FALSE}
##Subsetting the 431 samples that passed Darren's qc


#sed 's/FlowCell//g'  Downloads/hutt.500ht.qc_seq_lanes.txt |  awk '{print $3"_"$1"_"$2}' >Darrensqc
qc <- read.table("~/Dropbox/Darrensqc")
qccovt <- beforesumlanecov[which(beforesumlanecov$FC_findiv_lane%in%qc$V1),]
qclane <- beforesumlane[,which(colnames(beforesumlane)%in%qccovt$FC_findiv_lane)]

findivs <- sapply(strsplit(as.character(qccovt$findiv_lane), "_"), "[", 1)
qccov <- t(as.matrix(qccovt[,3:8]))
colnames(qccov) <- findivs
nms <- colnames(qccov)
combineqccov <- as.data.frame(t(qccov %*% sapply(unique(nms), "==", nms)))
#dim(combineqccov)
#combineqccov[1:5,1:5]

beforesumlaneqc <- qclane
var <- as.matrix(qclane)
findivs <- sapply(strsplit(colnames(qclane), "_"), "[", 2)
colnames(var) <- findivs
nms <- colnames(var)
aftersumlaneqc <- var %*% sapply(unique(nms), "==", nms)
#dim(aftersumlaneqc)
#aftersumlaneqc[1:5,1:5]

#write.table(aftersumlaneqc, "Gene_expression_4.14.16.txt", row.names = T, col.names = T, quote= F)
#write.table(combineqccov[,1:2], "Mappedreadcounts_4.14.16.txt", row.names = T, col.names = T, quote = F)

CS=colSums(aftersumlaneqc)
barplot(sort(CS), ylim=c(0, 50000000), main="Total mapped read counts")
abline(h=5000000, col="red")


counts1 = aftersumlaneqc+1
CPMqc = apply(counts1, 2, function(x) (x*1000000)/sum(x))
hist(log2(CPMqc), main = "CPM")
```



```{r CPMqc,  echo=FALSE, fig.height=5, fig.width=10, eval= FALSE}
###Subset of Darren's QC-ed samples that did not have 10 million reads

combineqccov[which(combineqccov$beforeWASP<10000000),]
...
####Still a lot below 10 million reads... if we subset the 431 individuals but combine technical replicates

findivqc <- colnames(aftersumlaneqc)
testtest <- aftersumlane[,which(colnames(aftersumlane)%in%findivqc)]
#dim(testtest)
#write.table(testtest, "Gene_expression_QC_4.14.16.txt", row.names = T, col.names = T, quote= F)
testtestcov<- aftersumlanecov[findivqc,]
#dim(testtestcov)
testtestcov[which(testtestcov$beforeWASP<10000000),]

counts1 = testtest+1
CPMqctest = apply(counts1, 2, function(x) (x*1000000)/sum(x))
hist(log2(CPMqctest), main = "CPM")

#write.table(CPMqctest, "Gene_expression_QC_CPM_4.14.16.txt", row.names = T, col.names = T, quote= F)
#file given to Xiang
...
gx.covars<-cbind.data.frame( sex,  readsafterWsex,  rnaconc, rin, batch, prep, conc, length, flowlane)
head(gx.covars)
...
```{r pccombine_aftercombat, echo = FALSE}
...
my_DGEList = DGEList(counts=res.gx.a)
...
```{r pcacheck_after_batch, echo = FALSE}
#source("https://bioconductor.org/biocLite.R")
#biocLite("sva")
library(sva)
...
```{r maternal and paternal, echo =FALSE}
#5-18-16 decided to only do TMM normalization but not regress out effects: 
# SO use my_data moving forward for now.

maternal<- read.table("~/star_overhang_genecount_genes_maternal", check.names = FALSE)
paternal<- read.table("~/star_overhang_genecount_genes_paternal", check.names = FALSE)
...
write.table(newerpat2, "Paternal_gene_normalized.txt", quote =F, row.names = T, col.names = T)
write.table(newermat2, "Maternal_gene_normalized.txt", quote =F, row.names = T, col.names = T)
...
keep <- read.table("~/star_overhang_genecount_hom", check.names = F)
k <- as.matrix(keep)
...
write.table(newerkeep, "Unknown_gene_normalized.txt", quote =F, row.names = T, col.names = T)
write.table(newer, "Total_gene_normalized.txt", quote = F, row.names = T, col.names = T)
